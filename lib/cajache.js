...
async function fncUse (id, fetchFnc, options = {}) {
	...
	if (options.condition) {
		...
		const shouldCache = options.condition(fresh);
		console.log( "shouldCache", `(${typeof shouldCache}): `, shouldCache);
		if (shouldCache !== true) return null;
 ...
 * @property {function} condition Function that will receive as argument the `fetchFnc` response. If it returns `true` the response will be cached, otherwise it won't be cached and `null` will be returned.
*/
