...
/**
 * @property {boolean} started Indicates if the expire watcher is running.
...
const expireWatcher = {
	...
	start: function(force = false) {
		...
		if (this.started && !force) return; // si ya está empezado, no hago nada
		this.started = true;
		...
		setTimeout( () => {
			...
			if (this.interval <= 0) return this.stop();
			this.start(true);
